//Bugs in calculating the number of work hours when in different days
//Instead of computing that manually, should use :
//  int hours = (int) start.until(stop, ChronoUnit.HOURS);
//Also, should not use ZonedDateTime, instead create a fake clock class
// and pass it to .now(clock) to facilitate tests.h
